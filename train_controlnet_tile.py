...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        default=64,
        help=(
            "The resolution for condition images, all the images in the train/validation dataset will be resized to this"
            " resolution"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--train_data_dir",
        type=str,
        default="/cephFS/video_lab/datasets/super_resolution/ffhq-dataset/images512x512_BLIPcaption/",
        help=(
...
class SDText2ImageDataset:
    def __init__(
        ...
        def transform(example):
            ...
            condition_image = TF.to_tensor(condition_image)
...
class LocalImageDataset(data.Dataset):
    def __init__(self, 
        ...
        self.img_paths = []
        self.img_paths.extend(sorted(glob.glob(f'{pngtxt_dir}/*.png'))[:])
...
def main(args):
    ...
    if args.tokenizer_name:
        ...
        print('xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx ',args.pretrained_model_name_or_path)
    ...
    train_dataset = LocalImageDataset(pngtxt_dir=args.train_data_dir,image_size=args.resolution, tokenizer=tokenizer, accelerator=accelerator, control_type='resize', null_text_ratio=args.proportion_empty_prompts, resize_bak=True)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                controlnet_image = batch["conditioning_pixel_values"].to(accelerator.device, dtype=weight_dtype, non_blocking=True)
